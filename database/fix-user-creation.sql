-- Fix for user profile creation issues
-- Run this in Supabase SQL editor

-- First, let's drop and recreate the trigger and function to fix any issues
...
-- Update RLS policies to be more permissive for user profile creation
...
CREATE POLICY "Service role can insert transactions" ON user_transactions
  FOR INSERT WITH CHECK (true);

-- Update usage history policies
DROP POLICY IF EXISTS "Users can insert own usage" ON user_usage_history;
CREATE POLICY "Users can insert own usage" ON user_usage_history
  FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Service role can insert usage" ON user_usage_history;
CREATE POLICY "Service role can insert usage" ON user_usage_history
  FOR INSERT WITH CHECK (true);

-- Grant necessary permissions
GRANT INSERT ON user_profiles TO authenticated;
GRANT INSERT ON user_transactions TO authenticated;
GRANT INSERT ON user_usage_history TO authenticated;

-- Also grant to service_role for triggers
GRANT ALL ON user_profiles TO service_role;
GRANT ALL ON user_transactions TO service_role;
GRANT ALL ON user_usage_history TO service_role;
GRANT ALL ON tier_configs TO service_role;
